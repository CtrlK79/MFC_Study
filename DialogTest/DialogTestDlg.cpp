...
BEGIN_MESSAGE_MAP(CDialogTestDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_Button_Modal, &CDialogTestDlg::OnBnClickedButtonModal)
	ON_BN_CLICKED(IDC_Button_Modeless, &CDialogTestDlg::OnBnClickedButtonModeless)
END_MESSAGE_MAP()


// CDialogTestDlg 메시지 처리기
...
void CDialogTestDlg::OnBnClickedButtonModal()
{
	CModalDlg Dlg;
	Dlg.DoModal();
}
...
void CDialogTestDlg::OnBnClickedButtonModeless()
{
	static CModelessDlg Dlg;
	if (Dlg.GetSafeHwnd() == NULL)
		Dlg.Create(IDD_Dialog_Modeless, this);
